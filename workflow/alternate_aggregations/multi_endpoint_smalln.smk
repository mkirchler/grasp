...
PRETRAIN_NAME = (
    "pretraining_smalln"  # or "baseline_pretraining"
)
SETTING_NAME = "attn_smalln"
...
SPEC_PARAMS = {
    ...
    # "n": [10_000, 25_000],
    "n": [10_000, 25_000, 50_000, 100_000, 200_000],
    "depth": [4],
    "embed_dim": [256],
    "sources": [ "condition_occurrence?procedure_occurrence?drug_exposure?extra" ],
    "provider": [ "openai_new", "rand", 'openai_onto' ],
    "input_dim": [None],
    "loss_type": ["cox"],
}
...
rule export_single_split:
    input:
        models=f"export/attn_smalln/attn_smalln.pkl",
    output:
        models='export/attn_smalln_single/attn_smalln_single.pkl',
    run:
        # models = pickle.load(open(f"export/attn_smalln/attn_smalln.pkl", "rb"))['models']
        # keys = [key for key in models.keys() if 'attn_smalln/0/' in key]
        # models = {key: models[key] for key in keys}
        # with open('export/attn_smalln_single/attn_smalln_single.pkl', "wb") as f:
        #     pickle.dump({"models": models}, f)

        models = pickle.load(open(input.models, "rb"))['models']
        keys = [key for key in models.keys() if 'attn_smalln/0/' in key]
        models = {key: models[key] for key in keys}
...
rule export_with_downstream:
    input:
        ...
        demo_baseline_downstream=expand(
            "results/{setting_name}/{avail}/demo_{n}/demo_{endpoint}.pkl",
            endpoint=ENDPOINTS,
            setting_name=SETTING_NAME,
            **SPEC_PARAMS,
        ),
    ...
    run:
        ...
        if "demo_baseline_downstream" in input.keys():
            for fn in input.demo_baseline_downstream:
                ...
                n = fn.split("/")[-2].split("_")[-1]
                specname = f"demo_baseline_{n}"
                recs.append(
        ...
        # plot_c_index_absolute(recs_b, output.b_plot)


rule aggregate_demo_endpoint:
    input:
        expand("models/demo_smalln/{{endpoint}}/{split}/{n}/{avail}/best_val_model.pkl", split=SPLITS, n="{n}", avail="{avail}"),
    output:
        f"results/{SETTING_NAME}/{{avail}}/demo_{{n}}/demo_{{endpoint}}.pkl",
    run:
...
module attention_pretraining:
    snakefile:
        "../training/attention_pretraining_from_omop_smalln.smk"
