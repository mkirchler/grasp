...
sweep_params = pd.DataFrame(
    [
        ...
        for max_depth in [2]  # or maybe up to 25?
        for num_boost_round in [500]
        for aft_loss_distribution_scale in [3.0]
        # for aft_loss_distribution_scale in [8.0]
        for ancestors in [5]
    ]
)
# sweep_params = pd.DataFrame(
...
rule export_single_split:
    input:
        models=f"export/prop_xgb_smalln/best_model.pkl",
    output:
        models="export/prop_xgb_smalln_single/best_model.pkl",
    run:
        with open(input.models, "rb") as f:
            models = pickle.load(f)
        models = [m for m in models if m["split"] == "0"]
...
rule full_export:
    input:
        results=expand(
            "results/prop_xgb_smalln/{endpoint}/n~{n}/avail~{avail}/{param}/model.pkl",
        ...
        models=expand(
            "models/prop_xgb_smalln/{endpoint}/{split}/n~{n}/avail~{avail}/{param}/model.pkl",
            split=SPLITS,
            param=sweep_paramspace.instance_patterns,
            endpoint=ENDPOINTS,
            avail=AVAIL,
    ...
    output:
        results=f"export/prop_xgb_smalln/results.pkl",
        c_plot=f"export/prop_xgb_smalln/c_plot.png",
        best_model=f"export/prop_xgb_smalln/best_model.pkl",
    default_target: True
    run:
        recs = []
        recs_CI_lower = []
...
rule aggregate_endpoint:
    ...
    output:
        f"results/prop_xgb_smalln/{{endpoint}}/n~{{n}}/avail~{{avail}}/{sweep_paramspace.wildcard_pattern}/model.pkl",
    run:
...
rule xgb_model_center_sweepstep:
    input:
        sparse_omop=paths.PROPAGATED_SPARSE_OMOP_SMALLN,
    ...
    output:
        f"models/prop_xgb_smalln/{{endpoint}}/{{split}}/n~{{n}}/avail~{{avail}}/{sweep_paramspace.wildcard_pattern}/model.pkl",
    params:
        rescale_age=True,
    run:
...
rule propagate_onto_labels_smalln:
    input:
        ...
        subset_train_iids="data/misc/smalln_splits/{split}_{n}.csv",
    output:
        paths.PROPAGATED_SPARSE_OMOP_SMALLN,
    run:
